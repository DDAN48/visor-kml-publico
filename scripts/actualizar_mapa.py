...
def clasificar_asignacion_caba(nombre_kml):
    """
    Devuelve metadata de asignación SOLO para KML de CABA.

    Reglas:
    - pending  -> Asignada
    - started / starting -> Iniciada
    - cualquier asignación fuera de CABA se omite
    - si no es un KML ETA de asignaciones, devuelve None
    """
    nombre_norm = normalizar_texto(nombre_kml)

    es_pending = "pending" in nombre_norm
    es_started = "started" in nombre_norm or "starting" in nombre_norm
    es_asignacion = es_pending or es_started

    if not es_asignacion:
        return None

    # Solo CABA. Aceptamos algunas variantes de nombre por robustez.
    es_caba = (
        "caba" in nombre_norm
        or "capital_federal" in nombre_norm
        or "capital federal" in nombre_norm
    )

    if not es_caba:
        return {
            "omitir": True,
            "motivo": "Asignación fuera de CABA"
        }
    ...
    for idx, archivo in enumerate(archivos_kml):
        ...
        layers.append({
            **capa_base,
            ...
            "static": False,
            "region_asignacion": capa_base.get("region_asignacion")
        })
